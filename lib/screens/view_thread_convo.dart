...
import 'package:web_tut/2.0/screens/thread_reply/thread_reply_widget.dart';
...
class ViewThreadConvo extends StatefulWidget {
  const ViewThreadConvo({Key key}) : super(key: key);

  @override
  State<ViewThreadConvo> createState() => _ViewThreadConvoState();
}

class _ViewThreadConvoState extends State<ViewThreadConvo> {
  ...
  //   }
  // }
  final viewThreadConvoControll = ScrollController();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: FutureBuilder<List<ThreadMessage>>(
        future: context.read<ReportProvider>().viewThreads(),
        builder: (context, snapshot) {
          ...
          if (snapshot.hasData) {
            ...
            return ListView.builder(
                controller: viewThreadConvoControll,
                ...
                itemBuilder: (context, index) {
                  ...
                  context.read<ThreadReplyProvider>().changeThreadID(
                      context.read<ReportProvider>().tappedReport.thrID);
                  context.read<ThreadReplyProvider>().changethrMessageID =
                      thrList[index].messageID;
                  return Column(
                    children: [
                      const SizedBox(
                        height: 12,
                          ...
                          Column(
                            children: [
                              ...
                              context
                                  ...
                                  // : Text('has data'),
                                  : ThreadReplyWidget(
                                      commentID: thrList[index].messageID,
                                      commentContent:
                                          thrList[index].messageContent,
                                      commenterToken:
                                          thrList[index].senderToken,
                                      myFullName: '',
                                      othersNames: [],
                                      likersTokens: [],
                                      myDeviceToken: '',
